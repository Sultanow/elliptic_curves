...
auto const & PQs(bool only_tested_and_ab_found = false) {
    static std::vector<std::pair<u64, u64>> pqs;// = {{3, 11}}; return pqs;
    static std::mutex mux;
    std::lock_guard lock(mux);
    if (!pqs.empty())
        return pqs;
    static auto const ab = [only_tested_and_ab_found]{
        std::set<std::pair<u32, u32>> solved_pq;
        if (!only_tested_and_ab_found)
            return solved_pq;
        std::ifstream f("ec_pq_find_ab_cases.log");
        ASSERT(f);
        std::regex re(R"(\s*\d+\:\s*(\d+)\,\s*(\d+)\s*(.*?)\s*)");
        std::string line;
        size_t total_cnt = 0, solved_cnt = 0;
        while (std::getline(f, line)) {
            std::smatch m;
            if (std::regex_match(line, m, re)) {
                auto p = std::stoll(m[1].str()), q = std::stoll(m[2].str());
                if (p > q)
                    std::swap(p, q);
                if (!m[3].str().empty()) {
                    solved_pq.insert({p, q});
                    ++solved_cnt;
                }
                ++total_cnt;
            }
        ...
        COUT(<< "AB total cnt " << total_cnt << ", solved cnt " << solved_cnt << std::endl);
        ASSERT(solved_cnt == solved_pq.size());
        return solved_pq;
    }();
    for (size_t pq = 1; pq < (1 << 21); ++pq) {
        ...
        if (fs[0] > fs[1])
            std::swap(fs[0], fs[1]);
        if (only_tested_and_ab_found && !ab.count({fs[0], fs[1]}))
            continue;
...
void Main() {
    ...
    
    {
        ...
        while (fsols >> p >> q >> iA >> iB >> iX >> xy_cnt) {
            ...
            ASSERT(fsols);
            if (!pqs_set.count({p, q}))
                continue;
            tested_pq.insert({p, q});
    ...
    if (only_tested_and_ab_found) {
        std::decay_t<decltype(pqs)> pqs2;
        for (auto const [p, q]: pqs)
            if (tested_pq.count({p, q}))
                pqs2.push_back({p, q});
        pqs = std::move(pqs2);
        pqs_set.clear();
        for (size_t i = 0; i < pqs.size(); ++i)
...

    {
        ...
        for (auto const [p, q]: pqs)
            if (!done_pq.count({p, q}))
                ++to_solve;
        COUT(<< "To solve " << to_solve << " PQs (total " << pqs.size() << ")" << std::endl);
    }
